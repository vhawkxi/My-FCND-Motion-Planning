...
# Method to prune path using collinearty function implemented below.
def prune_path(path):
    # while path length is >= 3 
    if len(path) >= 3:
        return path
    else:
        p = 2
        # We have no ida of how long the path segemnts will be so lets use while to loop through the solution
        while p < len(path) - 2:
...
## Implementation of colinear from course
